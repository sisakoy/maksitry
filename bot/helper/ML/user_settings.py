...
from bot import bot, user_data, config_dict, db
from bot.helper.ML.message.message_utils import sendMessage, editMessage, sendFile
...
from bot.helper.ML.other.utils import update_user_data, sync_to_async, new_thread
...
async def set_yt_options(client, message, pre_event):
    ...
    value = message.text
    update_user_data(user_id, 'yt_opt', value)
...
async def set_prefix(client, message, pre_event):
    ...
    handler_dict[user_id] = False
    value = message.text
    update_user_data(user_id, 'lprefix', value)
    await message.delete()
    await update_user_settings(pre_event)
    if db:
        await db.update_user_data(user_id)
...
async def set_thumb(client, message, pre_event):
    ...
    update_user_data(user_id, 'thumb', des_dir)
    await message.delete()
    await update_user_settings(pre_event)
    if db:
        await db.update_user_doc(user_id, 'thumb', des_dir)
...
async def add_rclone(client, message, pre_event):
    ...
    update_user_data(user_id, 'rclone', f'rclone/{user_id}.conf')
    await message.delete()
    await update_user_settings(pre_event)
    if db:
        await db.update_user_doc(user_id, 'rclone', des_dir)
...
async def leech_split_size(client, message, pre_event):
    ...
    update_user_data(user_id, 'split_size', value)
    await message.delete()
    await update_user_settings(pre_event)
    if db:
        await db.update_user_data(user_id)
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        update_user_data(user_id, 'as_doc',
        ...
        if await aiopath.exists(thumb_path):
            ...
            update_user_data(user_id, 'thumb', '')
            await update_user_settings(query)
            if db:
                await db.update_user_doc(user_id, 'thumb')
        else:
...
Check all yt-dlp api options from this <a href='https://github.com/yt-dlp/yt-dlp/blob/master/yt_dlp/YoutubeDL.py#L184'>FILE</a> or use this <a href='https://t.me/mltb_official/177'>script</a> to convert cli arguments to api options.
        ...
        handler_dict[user_id] = False
        await query.answer()
        update_user_data(user_id, 'yt_opt', '')
        await update_user_settings(query)
        if db:
            await db.update_user_data(user_id)
    elif data[2] == 'lss':
        ...
        handler_dict[user_id] = False
        await query.answer()
        update_user_data(user_id, 'split_size', '')
        await update_user_settings(query)
        if db:
            await db.update_user_data(user_id)
    elif data[2] == 'esplits':
        handler_dict[user_id] = False
        await query.answer()
        update_user_data(user_id, 'equal_splits',
        ...
        handler_dict[user_id] = False
        await query.answer()
        update_user_data(user_id, 'media_group',
        ...
        if await aiopath.exists(rclone_path):
            ...
            update_user_data(user_id, 'rclone', '')
            await update_user_settings(query)
            if db:
                await db.update_user_doc(user_id, 'rclone')
        else:
            await query.answer("Old Settings", show_alert=True)
            await update_user_settings(query)
    elif data[2] == 'lprefix':
        ...
        handler_dict[user_id] = False
        await query.answer()
        update_user_data(user_id, 'lprefix', '')
        await update_user_settings(query)
        if db:
            await db.update_user_data(user_id)
    elif data[2] == 'back':
...
bot.add_handler(MessageHandler(user_settings, filters=command(
    Commands.UserSetCommand) & CustomFilters.authorized))
