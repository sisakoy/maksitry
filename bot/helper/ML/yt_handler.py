...
from bot.helper.ML.other.utils import get_readable_file_size, is_url, new_task, sync_to_async, new_task, new_thread, get_readable_time
from bot.helper.ML.yt_helper.yt_downloader import YoutubeDLHelper
...
from bot.helper.ML.message.text import YT_HELP
...
async def _ytdl(client, message, isZip=False, isLeech=False, sameDir={}):
    ...
    if not is_url(link):
        await sendMessage(message, YT_HELP)
        return
...
bot.add_handler(MessageHandler(ytdl, filters=command(
    Commands.YtdlCommand) & CustomFilters.authorized))
bot.add_handler(MessageHandler(ytdlZip, filters=command(
    Commands.YtdlZipCommand) & CustomFilters.authorized))
bot.add_handler(MessageHandler(ytdlleech, filters=command(
    Commands.YtdlLC) & CustomFilters.authorized))
bot.add_handler(MessageHandler(ytdlZipleech, filters=command(
    Commands.YtdlZipLC) & CustomFilters.authorized))
