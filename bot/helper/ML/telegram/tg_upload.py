...
from bot import config_dict, user_data, GLOBAL_EXT_FILTER, bot, LOGGER
from bot.helper.ML.other.files_utils import clean_unwanted, is_archive, get_base_name
from bot.helper.ML.other.utils import sync_to_async
from bot.helper.ML.other.ffmpeg_utils import get_media_info, get_document_type, take_ss
    ...
    async def upload(self, o_files, m_size, size):
        ...
        for dirpath, _, files in sorted(await sync_to_async(walk, self.__path)):
            ...
            for file_ in natsorted(files):
                ...
                if file_.lower().endswith(tuple(GLOBAL_EXT_FILTER)):
