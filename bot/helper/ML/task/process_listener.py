...
from bot import Interval, aria2, download_dict, download_dict_lock, LOGGER, \
    config_dict, status_reply_dict_lock, user_data, non_queued_up, non_queued_dl, queued_up, \
    queued_dl, queue_dict_lock
from bot.helper.ML.other.utils import sync_to_async, get_readable_file_size
from bot.helper.ML.other.files_utils import get_base_name, get_path_size, clean_download, clean_target, \
    is_first_archive_split, is_archive, is_archive_split
from bot.helper.ML.other.ffmpeg_utils import split_file
from bot.helper.ML.other.exceptions import NotSupportedExtractionArchive
from bot.helper.ML.task.task_manager import start_from_queued, QueueStatus
from bot.helper.ML.status.process_status import ZipStatus, ExtractStatus, SplitStatus, RcloneStatus, TelegramStatus
from bot.helper.ML.telegram.tg_upload import TelegramUploader
from bot.helper.ML.rclone.rclone_upload import RcloneTransferHelper
from bot.helper.ML.message.message_utils import sendMessage, delete_all_messages, update_all_messages
...
class ProcessListener:
    def __init__(self, message, isZip=False, extract=False, isLeech=False, pswd=None, tag=None, seed=False, sameDir=None, rcFlags=None, upPath=None):
        if sameDir is None:
            sameDir = {}
    ...
    async def onDownloadComplete(self):
        ...
        if multi_links:
            ...
            return
        if name == "None" or not await aiopath.exists(f"{self.dir}/{name}"):
        ...
        if self.isLeech:
            ...
            tg = TelegramUploader(up_name, up_dir, self)
    ...
    async def onUploadComplete(self, link, size, files, folders, mime_type, name, rclonePath=''):
        ...
        if self.isLeech:
            ...
            LOGGER.info(f'Rclone Sending Link: {name} [{link}]')
            msg += f'\n\n<b>Type: </b>{mime_type}'
            if mime_type == "Folder":
                msg += f'\n<b>SubFolders: </b>{folders}'
                msg += f'\n<b>Files: </b>{files}'
            if link:
                LOGGER.info(f'Rclone Server Url Found')
                buttons = ButtonMaker()
                ...
                button = None
            msg += f'\n\n<b>cc: </b>{self.tag}'
            LOGGER.info(f'Sending Final Message : {name}')
