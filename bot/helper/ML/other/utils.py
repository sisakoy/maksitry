...
from bot import download_dict, download_dict_lock, botStartTime, user_data, config_dict, botloop
...
class setInterval:
    def __init__(self, interval, action):
        ...
        self.action = action
        self.task = botloop.create_task(self.__set_interval())
...
def get_readable_message():
    ...
    for download in list(download_dict.values())[STATUS_START:STATUS_LIMIT+STATUS_START]:
        ...
        if download.status() not in [Name.STATUS_SPLITTING, Name.STATUS_SEEDING]:
            ...
            if hasattr(download, 'seeders_num'):
                try:
                    ...
                    pass
        elif download.status() == Name.STATUS_SEEDING:
            ...
            msg += f"\n<b>Size: </b>{download.size()}"
        msg += f"\n<code>/{Commands.CancelCommand} {download.gid()}</code>\n\n"
    if len(msg) == 0:
        return None, None
    dl_speed = 0
    up_speed = 0
    for download in download_dict.values():
        ...
        if tstatus == Name.STATUS_DOWNLOADING:
            spd = download.speed()
            if 'K' in spd:
                dl_speed += float(spd.split('K')[0]) * 1024
            elif 'M' in spd:
                dl_speed += float(spd.split('M')[0]) * 1048576
        elif tstatus == Name.STATUS_UPLOADING:
            spd = download.speed()
            if 'K' in spd:
                up_speed += float(spd.split('K')[0]) * 1024
            elif 'M' in spd:
                up_speed += float(spd.split('M')[0]) * 1048576
        elif tstatus == Name.STATUS_SEEDING:
