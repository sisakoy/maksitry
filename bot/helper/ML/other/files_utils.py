...
from bot import aria2, LOGGER, config_dict, GLOBAL_EXT_FILTER
...
async def count_files_and_folders(path):
    ...
    for _, dirs, files in await sync_to_async(walk, path):
        ...
        for f in files:
            if f.endswith(tuple(GLOBAL_EXT_FILTER)):
                total_files -= 1
...
async def start_cleanup():
    LOGGER.info('Cleaning Downloads')
    try:
...
def exit_clean_up(signal, frame):
    try:
        ...
        clean_all()
        srun(['pkill', '-9', '-f', 'aria2c|rclone|ffmpeg'])
        sexit(0)
